...
class BaseBinarizer:
    ...
    def load_meta_data(self):
        ...
        for r in items_list:
            i = r['spk_id']
            if i == 0 or i == 1 or i == 3 or i == 4 or i == 5 or i == 6 or i == 7 or i == 9:
                continue
            num_id.append(i)
        for i in range(10):
            num = num_id.count(i)
    ...
    def process_data(self, prefix):
        ...
        if prefix == "all":
            ...
            for item in tqdm(items, desc="Processing items"):
                ...
                with torch.no_grad():
                    _, _, emo_VAD = model(pro_wav.to(device))
            ...
            for emo_id in tqdm(self.VAD_values):
                ...
                for emo_vad, item_name in self.VAD_values2[emo_id]:
                    ...
                    distance_to_neutral = pairwise_distance(emo_vad.unsqueeze(0), VAD_center, p=2)
                    all_distances.append(distance_to_neutral.item())
                
                self.q1[emo_id] = np.percentile(all_distances, 25)
                self.q3[emo_id] = np.percentile(all_distances, 75)
            ...
            VAD_I2I_Sad_center = torch.tensor([0.48519272, 0.57727589, 0.39524114]).to(device)
            VAD_I2I_Sur_center = torch.tensor([0.36877737, 0.48431942, 0.36644742]).to(device)
            with open('Spherical_coordinate_I2I_IQR.txt', 'w') as f:
                for item in tqdm(items, desc="Processing items"):
                    emo_id = item['emo_id']
                    wav_gt = torch.from_numpy(item['wav'])
                    wav_gt = wav_gt.unsqueeze(0)
                    ...
                    with torch.no_grad():
                        all_hidden_emotion_vector, hidden_emotion_vector, emo_VAD = model(pro_wav.to(device))

                    item['all_hidden_emotion_vector'] = all_hidden_emotion_vector.to("cpu")
                    item['hidden_emotion_vector'] = hidden_emotion_vector.to("cpu")
                    q1 = self.q1[emo_id]
                    q3 = self.q3[emo_id]
                    re_emo_VAD = emo_VAD - VAD_center
                    item['spherical_emotion_vector'] = self.convert_tensor_to_polar_coordinates(re_emo_VAD, q1, q3, emo_id).to("cpu")
                    
                    emo_VAD_list = item['spherical_emotion_vector'].squeeze().cpu().numpy().tolist()
                    f.write(f"{item['item_name']}, {emo_id}, {emo_VAD_list}\n")
    ...
    @classmethod
    def convert_tensor_to_polar_coordinates(cls, tensor, q1, q3, emo_id):
