...
class BASECFM(torch.nn.Module, ABC):
    ...
    def forward(self, mu, mask, n_timesteps, temperature=1.0, spks=None, guidance_scale=0.0):
        """Forward diffusion
        ...
        return self.solve_euler(
            z, t_span=t_span, mu=mu, mask=mask, spks=spks, guidance_scale=guidance_scale
        )

    def solve_euler(self, x, t_span, mu, mask, spks, guidance_scale=0.0):
        """
    ...
    def compute_loss(self, x1, mask, mu, spks=None, loss_mask=None):
        ...
        loss = F.mse_loss(
            self.estimator(y, mask, mu, t.squeeze(), spks)*mask, u*mask, reduction="sum"
...
class CFM(BASECFM):
    def __init__(
        ...
        self.estimator = Decoder(
            ...
            down_block_type=decoder_params["down_block_type"],
            mid_block_type=decoder_params["mid_block_type"],
            up_block_type=decoder_params["up_block_type"],
        )
